//
// FGL driver for LPrint, a Label Printer Utility
//
...
static const char * const lprint_fgl_media[] =
...
static int	lprint_fgl_print(lprint_printer_t *printer, lprint_job_t *job);
static int	lprint_fgl_status(lprint_printer_t *printer);


//
// 'lprintInitFGL()' - Initialize the driver.
//
    ...
    lprint_driver_t *driver)		// I - Driver
{
  ...
  driver->printfunc  = lprint_fgl_print;
  driver->statusfunc = lprint_fgl_status;
  ...
  driver->num_media = (int)(sizeof(lprint_fgl_media) / sizeof(lprint_fgl_media[0]));
...
//
// 'lprint_fgl_status()' - Get current printer status.
//
