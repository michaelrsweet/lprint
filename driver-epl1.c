//
// EPL1 driver for LPrint, a Label Printer Utility
//
...
static const char * const lprint_epl1_media[] =
...
static int	lprint_epl1_print(lprint_printer_t *printer, lprint_job_t *job);
static int	lprint_epl1_status(lprint_printer_t *printer);


//
// 'lprintInitEPL1()' - Initialize the driver.
//
    ...
    lprint_driver_t *driver)		// I - Driver
{
  ...
  driver->printfunc  = lprint_epl1_print;
  driver->statusfunc = lprint_epl1_status;
...
//
// 'lprint_epl1_print()' - Print a file.
//
...
//
// 'lprint_epl1_status()' - Get current printer status.
//
